...
def create_admin_keyboard():
    ...
    keyboard = [
        ...
        [KeyboardButton("🎨 إعدادات السمة"), KeyboardButton("🚀 الحملات التسويقية")],
        [KeyboardButton("💾 النسخ الاحتياطي"), KeyboardButton("❓ مساعدة المسؤول")],
...
def create_user_keyboard():
    ...
    keyboard = [
        ...
        [KeyboardButton("📋 قائمة الشحنات المستلمة"), KeyboardButton("❓ المساعدة")],
        [KeyboardButton("❌ إلغاء العملية الحالية"), KeyboardButton(st.MAIN_MENU_BUTTON)]
    ]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
def main():
    ...
    async def handle_photos(update, context):
        ...
        if 'conversation_state' in context.user_data:
            ...
            # لا توجد محادثة نشطة، رسالة افتراضية
            await update.message.reply_text("لم أفهم هذه الصورة. هل أنت في سياق إضافة إشعار؟")
